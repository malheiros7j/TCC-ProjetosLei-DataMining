...
sub verboPt {
    ...
    if (($word =~ /([aei][rsv]s*i*eis*|[aei]s[st]es*|[aei]rao)$/) && ($` =~ /^$self->{posV}/o)) {
        $word = $`;
    } elsif ($word =~ /([aei][rv]i*[aeo][ms]*|[aei][rv]*i*[ae]*mos*|[aei]ssemo*s*|ia[ms]*|[ae]is*|[aei]n*d[ao]s*)$/) {
        $stem = $`;
        if ($stem =~ /^$self->{posV}/o) {
            $word = $stem;
        } 
    } elsif ($word =~ /[aeio][srum]*$/) {
        $stem = $`;
        if ($stem =~ /^$self->{posV}/o) {
            $word = $stem;
        } 
